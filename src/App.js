...
// Features: Auth · Firestore · Storage · GPS · Mobile Money · Session Timeout
// =============================================================================
...
function SessionBanner({remaining}){
  if(!remaining||remaining>5*60*1000)return null;
  const mins=Math.floor(remaining/60000);
  const secs=Math.floor((remaining%60000)/1000);
  return <div style={{background:"#92400e",color:"#fef3c7",textAlign:"center",padding:"7px 16px",fontSize:12,display:"flex",alignItems:"center",justifyContent:"center",gap:8,fontWeight:600}}><Ic n="clock" s={13}/>Session expires in {mins}:{String(secs).padStart(2,"0")} — move your mouse to stay signed in</div>;
}
...
// ─── GPS FIELD COMPONENT ──────────────────────────────────────────────────────
function GPSField({gps,gpsLoading,gpsError,onCapture}){
  return <div style={{marginBottom:16}}>
    <label style={{display:"block",fontSize:11,color:"#4a7a58",marginBottom:6,textTransform:"uppercase",letterSpacing:"0.07em",fontWeight:700}}>GPS Location</label>
    <div style={{display:"flex",gap:8,alignItems:"flex-start"}}>
      <div style={{flex:1,background:"#f8faf8",border:"1px solid #d4e8d4",borderRadius:8,padding:"10px 14px",fontSize:13,color:gps?"#1a2e1a":"#9ab89a",minHeight:42,display:"flex",alignItems:"center",gap:6}}>
        {gpsLoading?<><div style={{width:12,height:12,border:"2px solid #1a5c2a",borderTopColor:"transparent",borderRadius:"50%",animation:"spin 0.8s linear infinite"}}/>Getting location…</>:
         gps?<><Ic n="map" s={13}/>{gps.lat.toFixed(6)}, {gps.lng.toFixed(6)} <span style={{color:"#9ab89a",fontSize:11}}>(±{gps.accuracy}m)</span></>:
         <><Ic n="map" s={13}/>Not captured</>}
      </div>
      <button type="button" onClick={onCapture} disabled={gpsLoading} style={{padding:"10px 14px",background:"linear-gradient(135deg,#1a5c2a,#2d8a45)",color:"#fff",border:"none",borderRadius:8,cursor:gpsLoading?"not-allowed":"pointer",fontSize:12,fontFamily:"inherit",fontWeight:600,display:"flex",alignItems:"center",gap:5,whiteSpace:"nowrap"}}><Ic n="map" s={13}/>{gps?"Re-capture":"Capture GPS"}</button>
    </div>
    {gps&&<a href={`https://maps.google.com/?q=${gps.lat},${gps.lng}`} target="_blank" rel="noreferrer" style={{fontSize:11,color:"#1a5c2a",textDecoration:"none",marginTop:4,display:"inline-flex",alignItems:"center",gap:3}}><Ic n="map" s={11}/>View on Google Maps</a>}
    {gpsError&&<div style={{fontSize:11,color:"#ef4444",marginTop:4}}>{gpsError}</div>}
  </div>;
}

// ─── IMAGE FIELD COMPONENT ────────────────────────────────────────────────────
function ImageField({label,preview,onChange,loading}){
  const fileRef=useRef();
  return <div style={{marginBottom:16}}>
    <label style={{display:"block",fontSize:11,color:"#4a7a58",marginBottom:6,textTransform:"uppercase",letterSpacing:"0.07em",fontWeight:700}}>{label}</label>
    <div style={{display:"flex",gap:10,alignItems:"center"}}>
      {preview?<img src={preview} alt="preview" style={{width:70,height:70,objectFit:"cover",borderRadius:8,border:"1px solid #d4e8d4"}}/>:
        <div style={{width:70,height:70,background:"#f0faf2",border:"2px dashed #c8e6c8",borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",color:"#9ab89a"}}><Ic n="camera" s={24}/></div>}
      <div>
        <button type="button" onClick={()=>fileRef.current.click()} disabled={loading} style={{padding:"8px 14px",background:"#f0faf2",color:"#1a5c2a",border:"1px solid #c8e6c8",borderRadius:8,cursor:"pointer",fontSize:12,fontFamily:"inherit",fontWeight:600,display:"flex",alignItems:"center",gap:5}}>{loading?<><div style={{width:11,height:11,border:"2px solid #1a5c2a",borderTopColor:"transparent",borderRadius:"50%",animation:"spin 0.8s linear infinite"}}/>Uploading…</>:<><Ic n="camera" s={13}/>{preview?"Change Photo":"Take / Upload Photo"}</>}</button>
        <div style={{fontSize:10,color:"#9ab89a",marginTop:4}}>JPG, PNG · Max 5MB</div>
        <input ref={fileRef} type="file" accept="image/*" capture="environment" style={{display:"none"}} onChange={onChange}/>
      </div>
    </div>
  </div>;
}
...
function LoginPage({addToast}){
  ...
  async function doLogin(){
    ...
    try{
      ...
      setErr(e.code==="auth/user-not-found"||e.code==="auth/wrong-password"||e.code==="auth/invalid-credential"?"Invalid email or password.":e.code==="auth/too-many-requests"?"Too many failed attempts. Wait a few minutes.":e.code==="auth/network-request-failed"?"No internet connection.":"Login failed: "+e.message);
    }
    setLoading(false);
  }
  ...
  return <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#f0faf2,#e8f5ec 50%,#f5f9f5)",display:"flex",alignItems:"center",justifyContent:"center",padding:16,fontFamily:"Outfit,sans-serif"}}>
    ...
    <div style={{width:"100%",maxWidth:420,animation:"fadeUp 0.5s ease"}}>
      ...
      <div style={{...card,padding:26}}>
        ...
        <button onClick={doLogin} disabled={loading||!email||!pass} style={{width:"100%",padding:"12px",background:loading||!email||!pass?"#9ab89a":"linear-gradient(135deg,#1a5c2a,#2d8a45)",border:"none",borderRadius:10,color:"#fff",fontSize:14,fontWeight:600,cursor:loading||!email||!pass?"not-allowed":"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>{loading?"Verifying…":<><Ic n="lock" s={15}/>Sign In Securely</>}</button>
      </div>
      <p style={{textAlign:"center",color:"#7a9a78",fontSize:11,marginTop:16}}>© 2024 Bekwai Municipal Assembly · Ashanti Region, Ghana</p>
    </div>
  </div>;
}
...
function Dashboard({ratepayers,payments,Officers,me}){
  ...
  const momoTotal=paid.filter(p=>p.paymentMethod==="Mobile Money").reduce((s,p)=>s+p.amount,0);
  const cashTotal=paid.filter(p=>p.paymentMethod!=="Mobile Money").reduce((s,p)=>s+p.amount,0);
  ...
  return <div>
    <div style={{background:"linear-gradient(135deg,#1a5c2a,#2d8a45)",borderRadius:16,padding:"20px 24px",marginBottom:24,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12,boxShadow:"0 4px 16px rgba(26,92,42,0.25)"}}>
      ...
      <div style={{textAlign:"right"}}><div style={{fontSize:11,color:"rgba(255,255,255,0.6)",textTransform:"uppercase",letterSpacing:"0.07em"}}>Your Collections</div><div style={{fontSize:26,fontWeight:700,color:"#fff",fontFamily:"Cormorant Garamond,serif"}}>{fmt(myTotal)}</div><div style={{fontSize:11,color:"rgba(255,255,255,0.6)",marginTop:2}}>{myCount} transaction{myCount!==1?"s":""}</div></div>
    </div>

    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:14,marginBottom:22}}>
      <StatCard label="Total Revenue"         value={fmt(totalR)}       icon="trend"  color="#16a34a" sub={`${paid.length} payments`}/>
      <StatCard label="Outstanding Levies"    value={fmt(totalO)}       icon="alert"  color="#d97706" sub={`${owing.length} unpaid`}/>
      ...
      {canDo(me,"iam")&&<StatCard label="Active Officers" value={Officers.filter(o=>o.status==="active").length} icon="shield" color="#b45309" sub="System users"/>}
    </div>
    ...
    <div style={{...card,padding:20,overflowX:"auto"}}>
      ...
      <table style={{width:"100%",borderCollapse:"collapse",minWidth:560}}>
        <thead><tr>{["Ratepayer","Levy Type","Amount","Method","Date","Officer"].map(h=><th key={h} style={{textAlign:"left",padding:"8px 12px",fontSize:11,color:"#7a9a78",textTransform:"uppercase",letterSpacing:"0.06em",borderBottom:"1px solid #f0f0f0",fontWeight:700}}>{h}</th>)}</tr></thead>
        <tbody>{recent.map(p=>{const rp=ratepayers.find(r=>r.id===p.ratepayerId);const off=Officers.find(o=>o.id===p.collectedBy);return <tr key={p.id}><td style={{padding:"10px 12px",fontSize:13,color:"#2a5a38",borderBottom:"1px solid #f8f8f8"}}>{rp?.name}</td><td style={{padding:"10px 12px",fontSize:13,color:"#2a5a38",borderBottom:"1px solid #f8f8f8"}}>{p.type}</td><td style={{padding:"10px 12px",fontSize:13,color:"#16a34a",borderBottom:"1px solid #f8f8f8",fontWeight:700}}>{fmt(p.amount)}</td><td style={{padding:"10px 12px",fontSize:12,borderBottom:"1px solid #f8f8f8"}}>{p.paymentMethod==="Mobile Money"?<span style={{background:"rgba(124,58,237,0.1)",color:"#7c3aed",padding:"2px 8px",borderRadius:20,fontSize:11}}>📱 MoMo</span>:<span style={{background:"rgba(3,105,161,0.1)",color:"#0369a1",padding:"2px 8px",borderRadius:20,fontSize:11}}>💵 Cash</span>}</td><td style={{padding:"10px 12px",fontSize:13,color:"#9ab89a",borderBottom:"1px solid #f8f8f8"}}>{fmtDate(p.date)}</td><td style={{padding:"10px 12px",fontSize:13,color:"#2a5a38",borderBottom:"1px solid #f8f8f8"}}>{off?.name||"—"}</td></tr>;})}
        </tbody>
      </table>}
    </div>
  </div>;
...
// =============================================================================
// RATEPAYERS — with GPS + Photo
// =============================================================================
function Ratepayers({ratepayers,me,addToast}){
  const [search,setSearch]=useState("");
  const [showForm,setShowForm]=useState(false);
  const [viewRp,setViewRp]=useState(null);
  const [saving,setSaving]=useState(false);
  const [imgLoading,setImgLoading]=useState(false);
  const [photoPreview,setPhotoPreview]=useState(null);
  const [photoFile,setPhotoFile]=useState(null);
  const {gps,gpsLoading,gpsError,capture}=useGPS();
  ...
  async function save(){
    ...
    try{
      ...
      if(photoFile){
        setImgLoading(true);
        photoURL=await uploadImage(photoFile,`ratepayers/${id}/photo`);
        setImgLoading(false);
      }
      const data={...form,id,pin,registeredDate:today(),status:"active",createdBy:me.id};
  ...
  return <div>
    ...
    <div style={{...card,overflow:"hidden",overflowX:"auto"}}>
      <table style={{width:"100%",borderCollapse:"collapse",minWidth:700}}>
        ...
        <tbody>{filtered.length===0?<tr><td colSpan={8} style={{textAlign:"center",padding:40,color:"#9ab89a"}}>No ratepayers found.</td></tr>:
          filtered.map(r=>{const cc=catC[r.category]||catC.Resident;return <tr key={r.id} style={{borderTop:"1px solid #f5f5f5",cursor:"pointer"}} onClick={()=>setViewRp(r)}>
            ...
            <td style={{padding:"10px 14px"}}>{r.gps?<span style={{background:"rgba(22,163,74,0.1)",color:"#16a34a",padding:"2px 8px",borderRadius:20,fontSize:11}}>📍 Yes</span>:<span style={{color:"#9ab89a",fontSize:11}}>—</span>}</td>
    ...
    {showForm&&<Modal title="Register New Ratepayer" onClose={()=>setShowForm(false)} wide>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
        ...
        <div>
          <ImageField label="Ratepayer Photo" preview={photoPreview} onChange={handlePhotoChange} loading={imgLoading}/>
          <GPSField gps={gps} gpsLoading={gpsLoading} gpsError={gpsError} onCapture={capture}/>
        </div>
      </div>
      <div style={{display:"flex",gap:12,marginTop:8}}>
        ...
        <button onClick={save} disabled={saving||imgLoading} style={{flex:2,padding:"11px",background:saving?"#9ab89a":"linear-gradient(135deg,#1a5c2a,#2d8a45)",border:"none",borderRadius:8,color:"#fff",cursor:saving?"not-allowed":"pointer",fontSize:14,fontWeight:600,fontFamily:"inherit"}}>{saving?"Saving…":"Register Ratepayer"}</button>
      </div>
    </Modal>}
    ...
    {viewRp&&<Modal title={viewRp.name} onClose={()=>setViewRp(null)} wide>
      <div style={{display:"grid",gridTemplateColumns:"auto 1fr",gap:20,alignItems:"start"}}>
        ...
        <div>
          <div style={{fontSize:11,color:"#4a7a58",textTransform:"uppercase",letterSpacing:"0.07em",fontWeight:700,marginBottom:4}}>PIN</div>
          <div style={{fontFamily:"monospace",color:"#1a5c2a",fontSize:15,marginBottom:12}}>{viewRp.pin}</div>
          {[["Ward",viewRp.ward],["Category",viewRp.category],["Phone",viewRp.phone],["Email",viewRp.email||"—"],["Address",viewRp.address],["Registered",fmtDate(viewRp.registeredDate)]].map(([l,v])=><div key={l} style={{display:"flex",gap:8,marginBottom:6}}><span style={{fontSize:12,color:"#7a9a78",width:80,flexShrink:0}}>{l}:</span><span style={{fontSize:13,color:"#1a2e1a",fontWeight:500}}>{v}</span></div>)}
        </div>
      </div>
      {viewRp.gps&&<div style={{marginTop:16,background:"#f0faf2",borderRadius:10,padding:"12px 14px",border:"1px solid #c8e6c8"}}>
        <div style={{fontSize:11,color:"#4a7a58",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.07em",marginBottom:6}}>📍 GPS Location</div>
        <div style={{fontSize:13,color:"#1a2e1a"}}>{viewRp.gps.lat.toFixed(6)}, {viewRp.gps.lng.toFixed(6)} <span style={{color:"#9ab89a",fontSize:11}}>(±{viewRp.gps.accuracy}m)</span></div>
        <a href={`https://maps.google.com/?q=${viewRp.gps.lat},${viewRp.gps.lng}`} target="_blank" rel="noreferrer" style={{fontSize:12,color:"#1a5c2a",textDecoration:"none",marginTop:4,display:"inline-flex",alignItems:"center",gap:4}}><Ic n="map" s={12}/>Open in Google Maps</a>
      </div>}
    </Modal>}
  </div>;
}
...
function Payments({payments,ratepayers,Officers,me,addToast}){
  ...
  async function confirmPay(p){
    ...
    try{
      ...
      await updateDoc(doc(db,"payments",p.id),{
        status:"paid", date:today(), receiptNo:rno, collectedBy:me.id,
        paymentMethod:payMethod,
        ...(payMethod==="Mobile Money"&&{momoNetwork,momoPhone,momoRef:momoRef||`MOMO-${Date.now()}`}),
      });
      const rp=ratepayers.find(r=>r.id===p.ratepayerId);
      ...
      addToast(`Payment saved. Receipt ${rno} issued.`,"success");
    }catch(e){ addToast(navigator.onLine?"Failed: "+e.message:"Saved offline","offline"); }
    setSaving(false);
  }
  ...
  return <div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:24,flexWrap:"wrap",gap:12}}>
      <div><h2 style={{color:"#1a2e1a",fontFamily:"Cormorant Garamond,serif",margin:"0 0 4px",fontSize:26}}>Levy &amp; Payment Tracker</h2><p style={{color:"#4a7a58",margin:0,fontSize:13}}>{payments.filter(p=>p.status==="paid").length} paid · {payments.filter(p=>p.status==="owing").length} outstanding · Cash &amp; Mobile Money</p></div>
    ...
    {showForm&&<Modal title="Issue Demand Notice" onClose={()=>setShowForm(false)}>
      ...
      <div style={{display:"flex",gap:12,marginTop:8}}>
        <button onClick={()=>setShowForm(false)} style={{flex:1,padding:"11px",background:"#f8faf8",border:"1px solid #e0ede0",borderRadius:8,color:"#4a7a58",cursor:"pointer",fontSize:14,fontFamily:"inherit"}}>Cancel</button>
        <button onClick={issueBill} disabled={saving} style={{flex:2,padding:"11px",background:saving?"#9ab89a":"linear-gradient(135deg,#1a5c2a,#2d8a45)",border:"none",borderRadius:8,color:"#fff",cursor:saving?"not-allowed":"pointer",fontSize:14,fontWeight:600,fontFamily:"inherit"}}>{saving?"Saving…":"Issue Notice"}</button>
      </div>
    </Modal>}
    ...
    {payModal&&(()=>{const rp=ratepayers.find(r=>r.id===payModal.ratepayerId);return <Modal title="Record Payment" onClose={()=>setPayModal(null)}>
      ...
      <div style={{marginBottom:16}}>
        <label style={{display:"block",fontSize:11,color:"#4a7a58",marginBottom:8,textTransform:"uppercase",letterSpacing:"0.07em",fontWeight:700}}>Payment Method</label>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          {["Cash","Mobile Money"].map(m=><button key={m} type="button" onClick={()=>setPayMethod(m)} style={{padding:"12px",border:`2px solid ${payMethod===m?"#1a5c2a":"#e0ede0"}`,borderRadius:10,background:payMethod===m?"#e8f5ec":"#fff",color:payMethod===m?"#1a5c2a":"#7a9a78",fontWeight:payMethod===m?700:400,cursor:"pointer",fontSize:13,fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>{m==="Cash"?"💵":"📱"}{m}</button>)}
        </div>
      </div>

      {payMethod==="Mobile Money"&&<div style={{background:"#f5f0ff",border:"1px solid #ddd6fe",borderRadius:10,padding:"14px",marginBottom:16}}>
        <div style={{fontSize:12,color:"#7c3aed",fontWeight:700,marginBottom:10}}>📱 Mobile Money Details</div>
        <Fld label="Network *"><select style={{...sel,borderColor:"#ddd6fe"}} value={momoNetwork} onChange={e=>setMomoNetwork(e.target.value)}>{MOMO_NETS.map(n=><option key={n}>{n}</option>)}</select></Fld>
        <Fld label="MoMo Phone Number *"><input style={{...inp,borderColor:"#ddd6fe"}} value={momoPhone} onChange={e=>setMomoPhone(e.target.value)} placeholder="e.g. 0244XXXXXX"/></Fld>
        <Fld label="Transaction Reference (optional)"><input style={{...inp,borderColor:"#ddd6fe"}} value={momoRef} onChange={e=>setMomoRef(e.target.value)} placeholder="e.g. ABC123456789"/></Fld>
        <div style={{fontSize:11,color:"#7c3aed",background:"#ede9fe",borderRadius:6,padding:"7px 10px"}}>Reference will be auto-generated if left blank</div>
      </div>}

      <div style={{background:"#f0faf2",border:"1px solid #c8e6c8",borderRadius:8,padding:"10px 14px",fontSize:12,color:"#1a5c2a",marginBottom:16}}>
        <div>Collected by: <strong>{me.name}</strong> ({me.staffId})</div>
        <div>Date: <strong>{fmtDate(today())}</strong></div>
      </div>
      <div style={{display:"flex",gap:12}}>
        ...
        <button onClick={()=>confirmPay(payModal)} disabled={saving} style={{flex:1,padding:"12px",background:saving?"#9ab89a":"linear-gradient(135deg,#166534,#16a34a)",border:"none",borderRadius:8,color:"#fff",cursor:saving?"not-allowed":"pointer",fontSize:14,fontWeight:600,fontFamily:"inherit"}}>{saving?"Saving…":"✓ Confirm Payment"}</button>
      </div>
    </Modal>;})()}
  </div>;
}
...
function Receipts({payments,ratepayers,Officers,addToast}){
  ...
  const filtered=paid.filter(p=>{
    const rp=ratepayers.find(r=>r.id===p.ratepayerId);
    const matchSearch=[rp?.name,p.receiptNo,p.type,p.period,rp?.ward].some(f=>f?.toLowerCase().includes(search.toLowerCase()));
    const matchMethod=filterMethod==="all"||(filterMethod==="momo"&&p.paymentMethod==="Mobile Money")||(filterMethod==="cash"&&p.paymentMethod!=="Mobile Money");
    return matchSearch&&matchMethod;
  });
  return <div>
    <div style={{marginBottom:24}}><h2 style={{color:"#1a2e1a",fontFamily:"Cormorant Garamond,serif",margin:"0 0 4px",fontSize:26}}>Revenue Receipts</h2><p style={{color:"#4a7a58",margin:0,fontSize:13}}>{paid.length} receipts · {paid.filter(p=>p.paymentMethod==="Mobile Money").length} via Mobile Money</p></div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:14}}>
      {filtered.length===0?<div style={{gridColumn:"1/-1",textAlign:"center",padding:60,color:"#9ab89a"}}>No receipts found.</div>:
        filtered.map(p=>{const rp=ratepayers.find(r=>r.id===p.ratepayerId);const off=Officers.find(o=>o.id===p.collectedBy);const isMomo=p.paymentMethod==="Mobile Money";return <div key={p.id} style={{...card,padding:18,border:`1px solid ${isMomo?"#ddd6fe":"#c8e6c8"}`}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:10}}>
            <span style={{background:isMomo?"rgba(124,58,237,0.1)":"rgba(22,163,74,0.1)",color:isMomo?"#7c3aed":"#16a34a",padding:"3px 10px",borderRadius:20,fontSize:10,fontWeight:700}}>{isMomo?"📱 MOMO":"✓ CASH"}</span>
  ...
  return <div>
    ...
    {sorted.length===0?<div style={{textAlign:"center",padding:70}}><div style={{fontSize:52,marginBottom:12}}>🎉</div><div style={{fontSize:20,fontFamily:"Cormorant Garamond,serif",color:"#1a5c2a"}}>All levies are paid up!</div></div>:
      sorted.map(([rpId,bills])=>{
        const rp=ratepayers.find(r=>r.id===rpId);const total=bills.reduce((s,b)=>s+b.amount,0);const hasOv=bills.some(b=>b.dueDate<today());
        return <div key={rpId} style={{...card,marginBottom:14,overflow:"hidden",border:`1px solid ${hasOv?"#fde68a":"#e8f0e8"}`}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 20px",background:hasOv?"#fffbeb":"#f8faf8",flexWrap:"wrap",gap:10}}>
            <div style={{display:"flex",alignItems:"center",gap:12}}>
              {rp?.photoURL&&<img src={rp.photoURL} alt="" style={{width:40,height:40,borderRadius:8,objectFit:"cover"}}/>}
              <div><div style={{color:"#1a2e1a",fontSize:16,fontWeight:700,fontFamily:"Cormorant Garamond,serif"}}>{rp?.name}</div><div style={{color:"#7a9a78",fontSize:12,marginTop:2}}>{rp?.pin} · {rp?.ward} · {rp?.phone}</div>{rp?.gps&&<a href={`https://maps.google.com/?q=${rp.gps.lat},${rp.gps.lng}`} target="_blank" rel="noreferrer" style={{fontSize:11,color:"#1a5c2a",textDecoration:"none",display:"inline-flex",alignItems:"center",gap:3,marginTop:2}}><Ic n="map" s={10}/>View on map</a>}</div>
            </div>
...
function IAMPage({Officers,me,addToast}){
  ...
  async function save(){
    ...
    try{
      if(editT){
        ...
        await logAudit(me.id,me.name,"OFFICER_UPDATED",`${form.name} updated by ${me.name}`);
        addToast(`${form.name} updated`,"success");
      }else{
        const cred=await createUserWithEmailAndPassword(auth,form.email,form.password);
        await setDoc(doc(db,"Officers",cred.user.uid),{id:cred.user.uid,name:form.name,email:form.email,phone:form.phone,role:form.role,ward:form.ward,staffId:form.staffId,status:"active",createdAt:today(),lastLogin:null});
        await logAudit(me.id,me.name,"OFFICER_REGISTERED",`${form.name} (${form.role}) registered by ${me.name}`);
        addToast(`Officer ${form.name} registered!`,"success");
      }
      setShowForm(false);
  ...
  return <div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:24,flexWrap:"wrap",gap:12}}>
      <div><h2 style={{color:"#1a2e1a",fontFamily:"Cormorant Garamond,serif",margin:"0 0 4px",fontSize:26}}>Officer Management</h2><p style={{color:"#4a7a58",margin:0,fontSize:13}}>Firebase Auth + Firestore · Real-time sync</p></div>
    ...
    {showForm&&<Modal title={editT?`Edit — ${editT.name}`:"Register New Officer"} onClose={()=>setShowForm(false)} wide>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        <Fld label="Phone"><input style={inp} value={form.phone} onChange={e=>setForm(p=>({...p,phone:e.target.value}))} placeholder="0244XXXXXX"/></Fld>
        <Fld label="Ward"><select style={sel} value={form.ward} onChange={e=>setForm(p=>({...p,ward:e.target.value}))}>{WARDS.map(w=><option key={w}>{w}</option>)}</select></Fld>
      </div>
...
export default function App(){
    ...
    const unsub=onAuthStateChanged(auth,async user=>{
      if(user){
        try{
          ...
          if(snap.exists()&&snap.data().status==="active"){setMe({id:snap.id,...snap.data()});setDataLoading(true);}
          else{await signOut(auth);}
        }catch(e){console.warn("Profile load failed:",e.message);}
      }else{
        setMe(null);setOfficers([]);setRatepayers([]);setPayments([]);setDataLoading(false);
  ...
  const resetSession=useCallback(()=>{
    ...
    sessionEnd.current=Date.now()+SESSION_TIMEOUT_MS;
    setSessionRemaining(SESSION_TIMEOUT_MS);
    sessionTimer.current=setTimeout(async()=>{
      await signOut(auth);
      addToast("Session expired after 30 minutes of inactivity. Please sign in again.","warning");
    },SESSION_TIMEOUT_MS);
    countdownTimer.current=setInterval(()=>{
      const remaining=sessionEnd.current-Date.now();
  ...
  useEffect(()=>{
    ...
    return()=>{
      ...
      events.forEach(e=>window.removeEventListener(e,resetSession));
    };
  },[me,resetSession]);
  ...
  function Sidebar(){return <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
    ...
    <div style={{padding:"12px 14px",borderTop:"1px solid #e8f0e8"}}>
      ...
      <div style={{marginTop:8,fontSize:10,color:"#9ab89a",textAlign:"center"}}>© 2024 Bekwai Municipal Assembly</div>
    </div>
  </div>;}

  return <div style={{display:"flex",minHeight:"100vh",background:"#f5f7f5",fontFamily:"Outfit,'Segoe UI',sans-serif",color:"#1a2e1a"}}>
    <style>{`@import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:wght@600;700&family=Outfit:wght@400;500;600;700&display=swap');*{box-sizing:border-box}::-webkit-scrollbar{width:4px;height:4px}::-webkit-scrollbar-track{background:#f0f0f0}::-webkit-scrollbar-thumb{background:rgba(26,92,42,0.2);border-radius:2px}@keyframes pulse{0%,100%{opacity:1}50%{opacity:0.4}}@keyframes slideIn{from{transform:translateX(20px);opacity:0}to{transform:translateX(0);opacity:1}}@keyframes spin{to{transform:rotate(360deg)}}@keyframes bar{0%{transform:translateX(-100%)}100%{transform:translateX(300%)}}input:focus,select:focus{border-color:#1a5c2a!important;box-shadow:0 0 0 3px rgba(26,92,42,0.1)!important;outline:none}button:active{transform:scale(0.98)}@media(max-width:768px){.dsk{display:none!important}.mtb{display:flex!important}.pad{padding:16px!important}}`}</style>
    <Toast toasts={toasts}/>
